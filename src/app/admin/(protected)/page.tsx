  ...
  return (
    <main className={styles.contentArea}>
      <div className={styles.pageHeading}>
        <div><p className={styles.eyebrow}>Editorial workspace</p><h1>Articles</h1><p>{items.length} localized draft{items.length === 1 ? "" : "s"}</p></div>
        <NewPostButton />
      </div>
      ...
      <div className={styles.tableWrap}>
        <table className={styles.articleTable}>
          ...
          <tbody>{items.map((item) => <tr key={item.id}>
            <td><strong>{item.title}</strong><span>/{item.locale}/blog/{item.slug}</span></td>
        ...
        {!items.length && <p className={styles.emptyState}>No articles match these filters.</p>}
      </div>
    </main>
  );
}
