...
export function ExternalMediaPlaceholder({
  compact = false,
  title,
  body,
}: {
  ...
  return (
    <div className={`${styles.placeholder} ${compact ? styles.compact : ""}`}>
      <div className={styles.copy}>
        <strong className={styles.title}>{title ?? t("externalBlockedTitle")}</strong>
        {!compact && <p className={styles.body}>{body ?? t("externalBlockedBody")}</p>}
      </div>
